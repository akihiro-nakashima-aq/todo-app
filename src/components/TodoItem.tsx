    ...
    <>
      <li className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200">
        ...
        <button
          ...
          className={`flex-1 text-left cursor-pointer hover:underline ${todo.completed ? "line-through text-gray-400" : "text-gray-800"}`}
        >
          {todo.title}
        </button>
      ...
      {modalOpen && <TodoDetailModal todo={todo} onClose={() => setModalOpen(false)} />}
    </>
  );
}
